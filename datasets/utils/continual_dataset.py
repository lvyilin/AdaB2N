...
class ContinualDataset:
    ...
    def get_task_order(self) -> list:
        """
        Returns the order of the tasks.
        :return: the order of the tasks
        """
        if self.args.seed == 0 or self.args.no_task_shuffle:
            return np.arange(self.N_TASKS * self.N_CLASSES_PER_TASK)
        rng = np.random.default_rng(42 + self.args.seed)
        order =  rng.permutation(self.N_TASKS * self.N_CLASSES_PER_TASK)
        print(f"Class indices are mapped from {np.arange(self.N_TASKS * self.N_CLASSES_PER_TASK)} to {order}")
        return order
