...
class RingBuffer:
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None):
        ...
        for i in range(examples.shape[0]):
            ...
            if index >= 0:
                self.examples[index] = examples[i].to(self.examples.device)
                if labels is not None:
                    self.labels[index] = labels[i].to(self.labels.device)
                if logits is not None:
                    self.logits[index] = logits[i].to(self.logits.device)
                if task_labels is not None:
                    self.task_labels[index] = task_labels[i].to(self.task_labels.device)

    def get_data(self, size: int, transform: nn.Module = None) -> Tuple:
        """
